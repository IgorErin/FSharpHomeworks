...
type Predicate(probability: float) =
    ...
    member this.Run() =
        Gen.frequency frequency |> Gen.sample 1 1 |> List.last
...
type InfectNet(adjacencyMatrix: bool[,], nodes: INode[]) =
    do
        // validate
        if Array2D.length1 adjacencyMatrix <> Array2D.length2 adjacencyMatrix then
            failwith "The matrix must be square"

        if Array2D.length1 adjacencyMatrix <> Array.length nodes then
            failwith "The length of the matrix must be equal to the number of nodes"

        nodes
        |> Array.distinctBy (fun node -> node.Id)
        |> Array.length
        |> fun length ->
            if length <> Array.length nodes then
                failwith "Node IDs should be different"

    /// create lookup table consisting of (node, his neighbors)
    let table =
        (adjacencyMatrix, nodes)
        ||> Array2D.mapByRow (fun isConnected node ->
            if isConnected then Some node else None
        )
